...
pub async fn summarize_messages(
    ...
    let summarize_req = AgentRequest {
        ...
        options: AgentOptions {
            ...
            effort: Some(crate::domain::agent::types::EffortLevel::Low),
    ...
    while let Some(ev) = stream.next().await {
        match ev? {
            ...
            crate::infrastructure::agent::provider::ProviderEvent::MessageComplete {
                message,
                stop_reason: sr,
            } => {
        ...
        for block in &msg.content {
            match block {
                ...
                Block::ToolResult {
                    ...
                    for c in content {
                        match c {
                            crate::domain::agent::types::ToolResultContent::Text { text } => {
                                out.push_str(text);
                            }
                            crate::domain::agent::types::ToolResultContent::Image { .. } => {
                                out.push_str("[image]");
                            }
                            crate::domain::agent::types::ToolResultContent::Json { raw } => {
