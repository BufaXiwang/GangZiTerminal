...
const BASELINE_STRATEGY_TEXT: &str = r#"龙头战法 — A 股模拟交易 Agent 默认策略（100 万 CNY 模拟账户）
...
二、情绪周期与仓位
冰点期（涨停<15家，连板消失，炸板率>50%）：空仓等待，最多 10% 试错仓
启动期（新题材首板集群，方向不明）：小仓参与，总仓位≤30%
主升期（涨停>20家，连板创新高，炸板率低）：重仓核心龙头，总仓位50%-80%
高潮期（龙头缩量加速，补涨股批量涨停）：逐步兑现，总仓位降至≤30%
退潮期（龙头断板/跌停，中位股A杀）：果断清仓，0% 仓位

三、龙头选股标准
- 板块内最早涨停、连板高度最高、封单最厚、分歧日抗跌最强
- 二板定龙头（Day 2 确认），三板确认地位
- 流通市值 20-200 亿，换手率 15%-35%，涨停时间早盘 10:30 前
- 题材最正宗、辨识度最高、有龙头历史记忆

四、买入条件
模式 A 打板：二板确认日/分歧转一致日，10:30 前封板，封单≥1万手，板块≥3只跟风涨停。首次 30%，确认后加至 50%
模式 B 低吸（龙回头）：龙头回调至 5 日线或前一涨停开盘价，缩量（<前日70%），不破前低。30%-50%
模式 C 弱转强：分歧日后次日高开≥2%+竞价放量+5分钟内封板。竞价 20%，封板加至 50%
...
九、不做什么
不做基本面投资（龙头战法是短线情绪博弈）、不做没涨停板的票、不做 ST/*ST、不做老题材回炒、不做消息不明的异动股、大盘跌幅>2% 或千股跌停时空仓。"#;
