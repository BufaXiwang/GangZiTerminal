...
use crate::domain::agent::strategy::{
    SignalCondition, Strategy, TargetRule, TriggerLogic,
};
...
pub fn seed_if_empty(app: &AppHandle) -> Result<(), String> {
    ...
    let momentum = Strategy {
        ...
        trigger_when: vec![
            SignalCondition { signal: SignalKind::BreakoutAbove20MA },
            SignalCondition { signal: SignalKind::VolumeSpike { ratio: 1.5 } },
            SignalCondition { signal: SignalKind::SectorStrengthAbove { pct: 3.0 } },
        ],
        ...
        ..Strategy::new(
            ...
            "".into(),
            vec![],
            TargetRule { direction: Direction::Up, pct_relative_to_current: 0.0, horizon_days: 0 },
            now,
        )
    };
    create(app, &momentum)?;
    ...
    let mean_reversion = Strategy {
        ...
        trigger_when: vec![
            SignalCondition { signal: SignalKind::RSIOversold { period: 14 } },
            SignalCondition { signal: SignalKind::BollingerBreakLower },
        ],
        trigger_logic: TriggerLogic::And,
        target: TargetRule {
            direction: Direction::Up,
    ...
    let capital_flow = Strategy {
        ...
        trigger_when: vec![
            SignalCondition { signal: SignalKind::OnDragonTigerList },
            SignalCondition { signal: SignalKind::NorthInflowStreak { days: 3 } },
        ],
        trigger_logic: TriggerLogic::And,
        target: TargetRule {
            direction: Direction::Up,
