//! Agent 机制健康度指标——v4 position-merged 版。
//!
//! **不是收益验证**——验证 agent 有效性的 ground truth 是模拟账户 PnL（主指标）。
//! 本模块只回答"机制是不是在按设计跑"。
//!
//! 6 个指标：
//! 1. Position 完整度：reasoning 非空 + signals_used ≥1 + take_profit 非空的比例
...
pub struct HealthMetrics {
    pub position_completeness_rate: Option<f64>,
    ...
    /// 今天新开的 position 数；连续多日为 0 → agent 没在产出新判断
...
pub fn compute(app: &AppHandle) -> Result<HealthMetrics, String> {
    ...
    // 1. Position 完整度：payload_json 内 reasoning 非空 + signalsUsed ≥1 + takeProfit 非空
    let (total_positions, complete_count): (u32, u32) = conn
        .query_row(
            "select
                count(*),
                sum(case
                    when length(json_extract(payload_json, '$.reasoning')) > 0
                     and json_array_length(coalesce(json_extract(payload_json, '$.signalsUsed'), '[]')) >= 1
                     and json_extract(payload_json, '$.takeProfit') is not null
                    then 1 else 0 end)
        ...
        .unwrap_or((0, 0));
    let position_completeness_rate = if total_positions > 0 {
        Some(complete_count as f64 / total_positions as f64)
    } else {
        None
    };

    let total_closed_positions: u32 = conn
        .query_row(
            "select count(*) from simulated_positions where status='closed'",
            [],
            |row| row.get(0),
        )
        .unwrap_or(0);
    ...
    // 北京时间 0 点的 RFC3339 等价（UTC = 北京时间 - 8h）
    ...
    let scan_ticks_today: u32 = conn
        ...
        .unwrap_or(0);
    let positions_opened_today: u32 = conn
        .query_row(
            "select count(*) from simulated_positions where created_at >= ?1",
            rusqlite::params![today_cutoff],
            |row| row.get(0),
        )
        .unwrap_or(0);
