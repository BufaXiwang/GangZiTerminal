//! News Tauri commands — `fetch_news` / `list_news_sources` / `refresh_news` / `warm_articles`。
//!
//! Spec: docs/design/news-module.md §4 / §5
//!
//! 注意：spec 中的 refresh_news / warm_articles 走 `{ ok: true | false }` 形状的响应，已经把
//! 失败语义编码在 payload 里；因此命令函数返回 `Result<T, CommandError>` 时只会在"调用方传参
//! 完全无法解析 / 内部 panic"这种意外路径才进 `Err`。正常 invalid_input 走 `ok = false`。

use crate::adapters::error::CommandError;
use crate::domain::news::types::{
    FetchNewsRequest, FetchNewsResponse, ListNewsSourcesResponse, RefreshNewsRequest,
    RefreshNewsResponse, WarmArticlesRequest, WarmArticlesResponse,
};
...
/// 手动刷新一次。失败码（invalid_input 等）走 `{ ok: false, error }` 形状。
///
/// Spec: news-module.md §4 refresh_news
#[tauri::command]
#[specta::specta]
pub async fn refresh_news(
    request: RefreshNewsRequest,
    service: State<'_, Arc<NewsService>>,
) -> Result<RefreshNewsResponse, CommandError> {
    let svc = service.inner().clone();
    Ok(svc.refresh_news(request).await)
}
