...
| PnL / 总资产 | 快照时算 | **不存** |
| 投资原则 | `principles` 表 | 见 [agent-redesign.md § 3.2](design/agent-redesign.md) |
...
|---|---|---|---|
| `chat` | user message / 用户指令 | Quotes + Account snapshot + News + 对话历史 + active Principles (top-N) + active Theses | 答复 + 可能的开/平/调仓 / create_thesis / propose_principle |
| `reflection` | 每日 15:30 Asia/Shanghai 自动 / Settings 手动触发 | 今日 closed positions + active theses | 对照 invalidation/validation 推进 thesis state + propose_principle |
...
| 账户写 | `open_position`（必填 thesis_id） / `close_position` / `scale_position` / `adjust_stops` | mutation |
| 资讯 | `search_news` | typed repository 读 DB |
| Thesis 写 | `create_thesis` / `update_thesis_state` / `attach_thesis_feedback` | mutation |
| Principle 写 | `propose_principle` / `confirm_principle` / `retire_principle` | mutation |

完整 spec 见 [docs/design/agent-redesign.md](design/agent-redesign.md)。

**Agent 触发模型**（v2 新增）：

| Trigger | 时机 | 行为 |
|---|---|---|
| `chat` / `user_message` / `user_instruction` | 用户在 chat 发消息 | 全功能 agent run |
| `scheduled` (close-15:30) | 每交易日 15:30 Asia/Shanghai 自动 | reflection 用例：对照 thesis invalidation/validation 提炼 principle |
...
| `SimulatedPosition` | `simulated_positions` (含 `thesis_id` 列) | `ACCOUNT_SNAPSHOT.positions` | SimAccount 写工具 | snapshot reads |
| `PositionEvent` | `position_events` | — | SimAccount 写工具 | `list_events` |
| `Thesis` | `theses` + `thesis_codes` + `thesis_events` | — | Thesis 工具 / scheduler reflection | Agent prompt / UI 围观 |
| `ChatMessage` | `chat_messages` | — | chat pipeline | Chat UI |
| `Principle` | `principles` | — | Principle 工具 / reflection | Agent prompt（按 hit_count + regime 过滤 top-N）|
| `AgentEpisode` | `agent_episodes` + `agent_episode_turns` | — | observer | 复盘 / Today 时间线 |
| `KvState` | `app_state` | — | Settings + scheduler | 全员 |

完整 schema 见 [docs/design/agent-redesign.md § 3](design/agent-redesign.md)。
...
Memory 是结构化 `Principle` 实体：

- 每条带 `state` (proposed/active/dormant/retired) + `origin` (user_stated/agent_inferred) +
  `hit_count` + `regime_tags`
- 注入 prompt：state=active + 按 regime 过滤 + hit_count 降序 top-25
- 写入流程：`propose_principle` → ≥3 hit 或用户复述 → `confirm_principle` → active
- 完整规则见 [agent-redesign.md § 5.2](design/agent-redesign.md)
...
│      ↓                                                        │
│  (6) Persistence + Audit                                      │
│      agent_episodes 写 audit row（含 trigger_kind / thesis_ids）│
│      │                                                        │
│      ↓                                                        │
│  (7) Feedback                                                 │
...
│      ↓                                                        │
│  (8) Memory                                                   │
│      propose_principle / confirm_principle / retire_principle  │
│      下次 prompt 自然带上新认知                                │
└──────────────────────────────────────────────────────────────┘
```
...
| 介入方式 | 实现 | 时机 |
|---|---|---|
| 给 agent 指导 | chat 消息 → agent 调 `propose_principle(origin=user_stated)` | 任何时候 |
...
│       ├── principle.rs             Principle aggregate (投资原则 / 已知偏差)
│       └── mod.rs
│
