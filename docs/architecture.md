...
>
> 历史版本见 git log；本次（2026-05-12）整体重写，新增 DDD-lite 结构 + snapshot-first 数据访问模式 + 5 项设计决策记录。

---

## 0. 一句话定位

> **Agent 自驱动的 A 股研究终端**——Agent 从市场数据 + 资讯里识别机会，在模拟账户里实盘验证，把过程沉淀成可学习的判断链。用户是观察者，不是审批员。
...
依赖规则（**Rust use 必须遵守**）：
- `quotes/` 不允许 use `agent | account | news | application`
- `account/` 只允许 use `quotes`；不允许 use `agent | news | application`
- `news/` 不允许 use `agent | account | quotes | application`
- `agent/` 通过 tool registry 调上面三个，不直接 use 内部细节
- `chat/`（application 入口 + frontend）通过订阅 events 看 agent；通过 commands 给 agent 发指令

### 1.4 持久化是命脉

任何 mutation **先写 event 再生效**。三层含义：
- **可审计**：每个开平仓、每个 memory 更新、每个 review 决策都有时间戳 + 来源标签
...
// === 写（mutation——briefing pipeline 调）===
...
**责任**：4 条 pipeline 触发 LLM run；通过 tool registry 调下面三个 source；产出决策（写 SimAccount + 更新 memory）；事件流通知 Chat。

**4 条 Pipeline**：

| Pipeline | 触发 | Source 输入（全部读 snapshot） | 动作 |
|---|---|---|---|
| `briefing` | scheduler（定时 + buffer 满）| News.pending + Quotes snapshot + Account snapshot | 出市场简报 + 开仓决策 |
| `review` | scheduler（到期复盘） | AnalysisRecord + Quotes + Account snapshot | 验证假设 + 平仓决策 |
| `refresh` | scheduler（行情自动刷新） | 由 Quotes / Account 各自调度，不在 Agent 层 | — |
| `chat` | user message | Quotes + Account snapshot + News + 对话历史 | 答复 + 可能的开/平/调仓 |

注：原来的 `refresh` pipeline 拆解到 Quotes / Account 各自的维护任务里——Agent 不再"驱动 refresh"。

**Agent 工具集**：

| 类别 | 工具 | snapshot vs fetch |
|---|---|---|
| 行情快照（同步） | `get_quote` / `get_market_overview` / `get_indicators` / `get_kline` | snapshot（不进则触发 ensure） |
...
| `NewsItem` | `news_items` | — | News refresh | Briefing / search |
...
| `AnalysisRecord` | `analysis_records` | — | Briefing / Review | Review 触发筛选 |
| `ChatMessage` | `chat_messages` | — | 全 pipeline 都写 | Chat UI |
| `InvestorMemory` | `app_state[KEY_MEMORY]` | `MEMORY_SNAPSHOT` | memory tools + pipelines | Agent prompt |
| `LearningProfile` | **派生**（不存） | `LEARNING_SNAPSHOT` | events 变化 / records 变化时重算 | Agent prompt |
...
- `learning_profile` = 从 records + events 现算
...
> 这一节是 LLM ↔ pipeline 之间的格式契约。Briefing / Review 是 JSON 协议；Chat 是
> Markdown + tool calls；Memory 有 merge 规则。
...
{
  "headline": "8-16 字精炼标题",
  "summaryMd": "Markdown 简报",
  "signals": [{ "theme": "...", "direction": "bullish|bearish|mixed|watch", "evidence": "..." }],
  "tradeCalls": [{
    "code": "600519",
    "name": "贵州茅台",
...
**约束**：
- `coveredNewsIds` 只含真的消化掉的资讯——未消化的 claimed 项会 revert 回 `pending`
- `tradeCalls` 可以为空
- 任何 `action: "buy"` **必须**同时给出 `triggerCondition` 和 `invalidationCondition`——否则 pipeline 降级为 watch，不开仓
- `highlight` 通常 null；high-importance 时单独作为 chat message 出现
...
**必填**：`summary` / `thesisStatus` / `evidence` / `checklistReview` / `learningUpdate`

**副作用**：若关联的 open 持仓被判 `invalidated`，review pipeline 在**单事务**里追加
`reviewed` + `invalidated` + `closed` 三条 event 并翻仓位状态（`db::commit_review`）；
失败整体回滚——record 保持未复盘、仓位保持 open。

Briefing 同样的单事务纪律（`db::commit_briefing`）：investor memory + analysis_records
+ simulated_positions + position_events + chat_messages + news status 全部原子提交。
...
- ❌ Frontend 直接调 model API 或拼 prompt——必须走 `agent::provider::ChatProvider`
...
│                       │                                       │
│  (1) Trigger          │                                       │
│      scheduler tick / user chat message                       │
│      │                                                        │
│      ↓                                                        │
...
│      Schedule: review 到期复盘                                 │
│      │                                                        │
│      ↓                                                        │
│  (8) Learning                                                 │
│      update_memory tool 更新长期记忆                           │
│      LearningProfile snapshot 重算                            │
│      下次 prompt 自然带上新认知                                │
└──────────────────────────────────────────────────────────────┘
```
...
2. update state snapshot       → simulated_positions / news_items.status
3. rebuild in-memory snapshot  → ACCOUNT_SNAPSHOT / NEWS_PENDING_CACHE
...
进程崩溃后重启：
- ✅ stocks 表已有 → 跳过刷新
- ✅ 半提交的 briefing.processing news → 自动 revert 到 pending（`recover_stale_processing`）
...
- 哪个 pipeline（briefing / review / chat）
- 哪个 LLM run（agent_run_id）
- 关联的 records / news / chat_message
...
| Gap | 现状 | 目标 |
|---|---|---|
| Snapshot-first 数据访问 | agent 直接调 fetch_* | 改成读 snapshot；snapshot 由各模块自治 |
| Account 聚合根 | 自由函数散在 `account.rs` | 收进 `Account` 聚合根 + private 字段 |
| Newtype IDs | 都用 `String` | `PositionId / StockCode / NewsId / AnalysisRecordId` |
| Account 模块独立目录 | `account.rs` 单文件 | `src-tauri/src/account/` 子目录 + 子模块拆分 |
| News 模块独立目录 | 散在 `news.rs` + `article/` | 整合到 `src-tauri/src/news/` |
| DDD 分层结构 | 散在 `src-tauri/src/` 根 | 分 `domain / infrastructure / application / adapters` |
| Quotes 全局 AppHandle | OnceLock hack | 显式参数 |
| `indicators_at_open` 持久化 | 未实现 | open 时算并存 PositionEvent payload |
| LearningProfile 派生输入 | 只用 records | 加 position_events 反馈 |
...
│   │   ├── source_tag.rs            SourceTag enum
│   │   └── mod.rs
│   ├── news/                        News Bounded Context
│   │   ├── item.rs                  NewsItem 实体
│   │   ├── article.rs               ArticleContent VO
│   │   ├── status.rs                NewsStatus + 状态转移规则
│   │   └── mod.rs
│   └── agent/                       Agent Bounded Context
│       ├── memory.rs                InvestorMemory VO
│       ├── analysis.rs              AnalysisRecord 聚合
│       ├── learning.rs              LearningProfile（派生）
│       └── mod.rs
│
├── infrastructure/                   ← I/O 实现，实现 domain 定义的 trait
...
│   ├── chart/                       图表标注 I/O（Phase 2 待接入）
│   ├── newsnow/                     NewsNow / RSS fetcher
...
│   ├── tauri_commands.rs            #[tauri::command] 函数
│   ├── agent_events.rs              AgentEvent → frontend event
│   ├── tool_registry.rs             LLM tools → domain API（anti-corruption）
│   └── mod.rs
│
└── main.rs
```
...
pub struct AnalysisRecordId(String);
...
pub struct Account {
    initial_cash: Money,
    positions: HashMap<PositionId, Position>,      // private
    events: Vec<PositionEvent>,                     // private（视为 in-memory log，落盘由 repo）
}

impl Account {
    pub fn load_from(events: Vec<PositionEvent>, positions: Vec<Position>) -> Self { ... }

    pub fn open(&mut self, req: OpenRequest, source: SourceTag, indicators: Option<IndicatorSnapshot>)
        -> Result<&Position, AccountError>
    {
        // 1. 校验规则（rules::ensure_*）
        // 2. 生成 events（PositionEvent::Opened）
        // 3. 应用到 positions + cash
        // 4. 返回 immutable 引用
    }
    pub fn close(&mut self, id: &PositionId, exit_price: Money, reason: CloseReason, source: SourceTag) -> Result<&Position, AccountError> { ... }
    pub fn scale(&mut self, id: &PositionId, delta: Shares, price: Money, thesis: String, source: SourceTag) -> Result<&Position, AccountError> { ... }
    pub fn adjust_stops(&mut self, id: &PositionId, sl: Option<Money>, tp: Option<Money>, ts: Option<DateTime>, source: SourceTag) -> Result<&Position, AccountError> { ... }
    pub fn cash(&self) -> Money { ... }              // derive from events
    pub fn open_positions(&self) -> Vec<&Position> { ... }
    pub fn pending_events(&self) -> &[PositionEvent] { ... }  // 给 repo 落盘用
}
```

**关键设计**：
- `positions` / `events` 是 private——外面无法 `&mut Position` 直接改字段
- 所有 mutation 走 Account 方法，方法内强制走"校验 → 生成 event → 应用"流程
- `pending_events()` 暴露未落盘的事件给 repo 取走 → repo 落盘 → 清空

### 9.5 Repository Pattern（按需引入）

不强制，但建议在引入 unit test 时定义：

```rust
// domain/account/mod.rs
#[async_trait]
pub trait AccountRepository {
    async fn load(&self) -> Result<Account, AccountError>;
    async fn save(&self, account: &mut Account) -> Result<(), AccountError>;  // 取 pending_events 落盘
}

// infrastructure/sqlite/account_repo.rs
pub struct SqliteAccountRepo { app: AppHandle }
#[async_trait]
impl AccountRepository for SqliteAccountRepo {
    async fn load(&self) -> Result<Account, AccountError> {
        let events = db::list_position_events(&self.app)?;
        let positions = db::list_simulated_positions(&self.app)?;
        Ok(Account::load_from(events, positions))
    }
    async fn save(&self, account: &mut Account) -> Result<(), AccountError> {
        // 单事务：写 pending events + replace positions
    }
}
```
...
- LLM 在 briefing 时能自己识别——这正是 agent 的工作
