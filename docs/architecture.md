...
#### Heuristic 模型（v3 取代 Principle）

Memory 是结构化 `Heuristic` 实体：

- 每条带 `effective_state` (active/challenged/probationary/dormant/retired) +
  `origin` (seed/user_stated/agent_inferred) + `category` (principle/known_bias/risk_preference) +
  `regime_tags` + `hit_count` / `miss_count` + `supporting_lesson_ids`
- `confidence()` 现算：`hit / (hit + miss)`，**不存**——track record 改变自动重算
- 注入 prompt：effective_state ∈ {active, probationary} + 按 regime 过滤 + confidence × application_count 降序 top-25
- 写入流程：
  - **seed**：启动时种 10 条经典 heuristic（如"高换手率板块易回调"）
...
│      ↓                                                        │
│  (8) Memory                                                   │
│      propose_heuristic → Lesson emerge → confidence 重算       │
│      下次 prompt 自然带上新认知                                │
└──────────────────────────────────────────────────────────────┘
```
...
| 介入方式 | 实现 | 时机 |
|---|---|---|
| 给 agent 指导 | chat 消息 → agent 调 `propose_heuristic(origin=user_stated)` | 任何时候 |
