...
type Price = number;       // CNY / share
type Shares = number;      // 股数，Account 股票交易要求 100 股整数倍
...
- Account 写接口必须校验 `Shares` 为正数；A 股股票买卖数量必须是 100 股整数倍。
...
- `eventId` 唯一标识一次事件事实。
- `correlationId` 串联一次用户请求、后台 run 或跨模块流程。
- `causationId` 指向直接导致本事件的上游 event / command / run。
- 生产者只表达事实，不指定消费者。
